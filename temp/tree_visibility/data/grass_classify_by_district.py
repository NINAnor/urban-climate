...
def print_unique_values(map_layer, attribute):
    ...
    distinct_values = gs.read_command(
        'db.select', 
        sql=f"SELECT DISTINCT {attribute} FROM {map_layer}"
    ).splitlines()
    
    # Print the number of unique values
    print(f"The number of unique {attribute} values in {map_layer} is {len(distinct_values)}")
...
def rename_cols(map_layer):
    ...
    for column in columns:
        ...
        if column_name.startswith('a_') and column_name not in exclude_columns:
            ...
            gs.run_command('v.db.renamecolumn', map=map_layer, column=(column_name,new_column_name))
...
def calc_area(map_layers):
    ...
    for i, map_layer in enumerate(map_layers):
        ...
        gs.run_command('v.to.db', overwrite=True, map=map, option='area', columns='area_sqm', unit='meters')



def classify_by_largest_overlap(original_map_layer, intersected_map_layer, col_id, col_district, col_area):
...
if __name__ == '__main__':
    ...
    classify_by_largest_overlap(original_map_layer, intersected_map_layer, col_id, col_district, col_area)
